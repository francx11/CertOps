...
def run_exam(questions: list, time_limit_minutes: int,
             pass_threshold: float = 0.70) -> dict | None:
    total = len(questions)
    recorded: dict = {}
...
def _list_certs(bank_root: Path) -> list:
    if not bank_root.exists():
        return []
    return sorted(p.name for p in bank_root.iterdir()
...
def main():
    ...
    ap.add_argument('--cert', required=True,
                    help='Certification slug, e.g. aws-ai-practitioner')
    ap.add_argument('--mode', choices=['quiz', 'exam'], default='quiz',
    ...
    ap.add_argument('--time-limit', type=int, default=90,
                    help='Exam time limit in minutes (default: 90)')
    ap.add_argument('--no-explanations', action='store_true',
                    help='Hide explanations after each quiz answer')
    ap.add_argument('--bank-root', default=None,
                    help='Override path to certifications/ directory')
    ap.add_argument('--list-certs', action='store_true',
                    help='List available certifications and exit')
    args = ap.parse_args()
